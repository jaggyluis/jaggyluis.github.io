...
(() => {
  ...
  var lscroll = document.getElementById("left-scroll-pane");

  var bottom = document.getElementById("bottom");
  var bottomHeader = document.getElementById("bottom-header");
  var bscroll = document.getElementById("bottom-scroll-pane");
  ...
  dropFile(document.getElementsByTagName('body')[0], ['json'], (fileData, fileName) => {
    ...
    if (json.type !== "SSDataSchema" || mapSchema !== null) {
      return;
    }

    mapSchema = json; // singleton ---
    mapOptions = json.options || {}; // global ---

    if (mapController === null) {
      mapController = new MapController(map, mapOptions);
  ...
  function loadDataLayer(data, label, options) {
    ...
    if (!(mapOptions && mapOptions.download === false)) {
    ...
    if (mapOptions && mapOptions.active === label) {
    ...
    //setTimeout(() => {
      ...
      // if (options.active) {
      //   mapController.selectProperty(label, options.active);
      // }
