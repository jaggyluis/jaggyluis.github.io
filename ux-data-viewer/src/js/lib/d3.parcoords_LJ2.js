...
var pc = function(selection, label) {
...
//LH from https://rawgit.com/mcwillso/parallel-coordinates/Column-Fields/d3.parcoords.js
function dimensionValListener(d) {
    ...
    var input = form
        .attr({
            "x": xy.x,
            "y": d3.transform(el.attr('transform')).translate[1]-(h-1),
            "height": h,
            "width": w
        })
        .append("xhtml:div")
        ...
        .on("blur", function () {
            updateMaxMinExtents(d, el, p_el, input.node().value);
        })
...
function addMaxMinText(g) {
    g.selectAll(".axis-max").remove();
    g.selectAll(".axis-min").remove();
    ...
    g.append("svg:text")
        ...
        .attr("class", function(d) {
            ...
            if (__.types[d] !== undefined && __.types[d] === "number") {

              return "axis-max " + d;

            } else {

              return "axis-category " + d;
            }
        })
        ...
        .on("click", function (d) {
          ...
          if (__.types[d] !== undefined && __.types[d] === "number") {
            ...
            return (dimensionValListener.bind(this))(d);
...
pc.updateAxes = function() {
  ...
  Object.keys(__.dimensions).forEach(d => {
    yscale[d] = pc.defaultScales()[__.dimensions[d].type](d);
  })
  ...
  function brushFor(axis) {
    ...
    brush
      ...
      .on("brush", function(d) {
        ...
        //LH from https://rawgit.com/mcwillso/parallel-coordinates/Column-Fields/d3.parcoords.js
        var maxs = d3.selectAll('.axis-max.' + d)[0];
        var mins = d3.selectAll('.axis-min.' + d)[0];
        ...
        if (brushed === -1) {
          ...
          if (yscale[d].domain().length > 1) {
            ...
            d3.select(maxs[0]).text((yscale[d].domain()[1]).toFixed(2));
            d3.select(mins[0]).text((yscale[d].domain()[0]).toFixed(2));

          } else {

            d3.select(maxs[0]).text((yscale[d].domain()[0]).toFixed(2));
            d3.select(mins[0]).text((yscale[d].domain()[0]).toFixed(2));
          }
        } else {

          d3.select(maxs[0]).text((+pc.brushExtents()[d][1]).toFixed(2));
          d3.select(mins[0]).text((+pc.brushExtents()[d][0]).toFixed(2));
        }
      ...
      .on("brushend", function(d) {
        events.brushend.call(pc, __.brushed, axis);
      });
